...
contract KernelFactoryS {
    KernelS public immutable kernelTemplate;
    ...
    constructor(IEntryPoint _entryPoint) {
        kernelTemplate = new KernelS(_entryPoint);
    }
    ...
    function createAccount(
            ...
            keccak256(
                abi.encodePacked(
                    ...
                    abi.encode(
                        ...
                        abi.encodeCall(KernelS.initialize, (_owner))
                    )
                )
            )
        );
        ...
        proxy = new EIP1967Proxy{salt: salt}(
            address(kernelTemplate),
            abi.encodeWithSelector(KernelS.initialize.selector, _owner)
        );
